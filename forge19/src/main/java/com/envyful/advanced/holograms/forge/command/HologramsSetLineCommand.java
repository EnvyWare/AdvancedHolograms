...
public class HologramsSetLineCommand {
    ...
    public void setLineCommand(@Sender ServerPlayer sender, String[] args) {
        if (args.length < 3) {
            sender.sendSystemMessage(Component.literal("§4Cannot find a hologram with that id. §7/hd setline <id> <line#> <text>"));
            return;
        }
        ...
        if (lineNum < 1) {
            sender.sendSystemMessage(Component.literal("§4Line number must be positive. §7/hd setline <id> <line#> <text>"));
            return;
        }
        ...
        sender.sendSystemMessage(Component.literal("§eSetting line to " + UtilChatColour.translateColourCodes('&', line) + " on " + args[0]));
    }
}
